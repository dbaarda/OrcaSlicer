...
{
    BoundingBox bbox = extrusion_polyline_extents(extrusion_path.polyline.to_polyline(), scaled(0.5 * extrusion_path.width));
    BoundingBoxf bboxf;
    if (! empty(bbox)) {
...
static inline BoundingBoxf extrusionentity_extents(const ExtrusionLoop &extrusion_loop)
{
    ...
    for (const ExtrusionPath &extrusion_path : extrusion_loop.paths)
        bbox.merge(extrusion_polyline_extents(extrusion_path.polyline.to_polyline(), scaled(0.5 * extrusion_path.width)));
    BoundingBoxf bboxf;
    if (! empty(bbox)) {
        bboxf.min = unscale(bbox.min);
        bboxf.max = unscale(bbox.max);
